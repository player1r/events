...
if ($admin == '1') {
    ...
    $name_event = $place = $address = $about = $form_event = $type_event = $scale_event = $sections_number = $date = $time = '';

    $name_event = filter_var(trim($_POST['name']),FILTER_SANITIZE_STRING);
    ...
    for ($i = 0; $i < count($_POST['name_section']); $i++)
    {
        $name_rus = $_POST['name_section'][$i];
        if($name_rus != '') 
        {
            $mysql->query("INSERT INTO `section_event` (`name_rus`,`id_event`) VALUES ('$name_rus','$id_event')");
        }
    }

    $mysql->query("INSERT INTO `events` (`id`, `name`, `about`, `place`, `address`, `type`, `scale`, `form`,`id_organization`, `background_image`, `date`, `time`, `status`) VALUES ('$id_event', '$name_event', '$about', '$place', '$address', '$type_event', '$scale_event', '$form_event', '$id_user', '$name', '$date', '$time', '1');");
